...
set "this_script=%userprofile%\My Drive\Documents\Tech\others\script-sync-ventoy-drive.cmd"

rem how to use : copy paste this script to root folder of pendrive

rem the batch will create an identity file to mark it as amr-drive

if not exist "%~dp0amr-drive.txt" ( echo. > "%~dp0amr-drive.txt" )

rem the script updates itself from the main source at my drive
cd "%~dp0."
if exist "%~dp0script-sync-ventoy-drive.cmd" (
	robocopy "%automata_path%\others" "%~dp0." "script-sync-ventoy-drive.cmd" /z /R:1 /W:1 /NDL /NJH /NJS /NS /NFL
)

rem create the tech folder in usbdrive
if not exist "%~dp0Tech" mkdir "%~dp0Tech"

rem run the local repo updater
if exist "%automata_path%\%automata_file%" (
	cd /d "%automata_path%"
	call "%automata_path%\%automata_file%"
)

REM Mirror all ISOs from microsoft iso source
if not exist "%destination_m%" mkdir "%destination_m%"
robocopy "%source_microsoft%" "%destination_m%" "*.iso" /purge /NDL /NJH /NJS /NS /Z /R:1 /W:1

REM Mirror linuxmint ISOs from linux iso source
